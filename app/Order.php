...
class Order extends Model
{
    ...
    protected $fillable = ['restorant_id', 'coupon','discount','fee', 'fee_value', 'static_fee', 'vatvalue','payment_info','mollie_payment_key','whatsapp_address','md','cart_storage_id','propina','number_people'];
    ...
    public function detailrestorant()
    {
    ...
    {
        return $this->belongsToMany(\App\Items::class, 'order_has_items', 'order_id', 'item_id')->withPivot(['qty', 'item_status','cart_item_id','item_observacion','extras', 'vat', 'vatvalue', 'variant_price', 'variant_name', 'id', 'discount'])->withTrashed();
    ...
    private function getOwnerOrderActions(){
        ...
        if(config('app.isft')){
            if(in_array($lastStatusAlias,["just_created"])){
                ...
                if(in_array($lastStatusAlias,["assigned_to_driver"]) && $this->delivery_method=="1" && !auth()->user()->hasRole('staff') ){
                ...
                if($this->delivery_method=="1" && !auth()->user()->hasRole('staff') ){
                    return ["buttons"=>['rejected_by_restaurant','delivered','assigned_to_driver'],'message'=>""];
                }
