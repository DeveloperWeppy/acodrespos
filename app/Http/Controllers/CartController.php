...
class CartController extends Controller
{
    ...
    public function add(Request $request)
    {
        ...
        if ($item && $canAdd) {
            ...
            Cart::add((new \DateTime())->getTimestamp(), $cartItemName, $cartItemPrice, $request->quantity, $request->personaccount, ['id'=>$item->id, 'variant'=>$request->variantID, 'extras'=>$request->extras, 'restorant_id'=>$restID, 'image'=>$item->icon,'discount'=>Money($cartItemPrice+$discount, config('settings.cashier_currency'), config('settings.do_convertion'))->format(), 'friendly_price'=>  Money($cartItemPrice, config('settings.cashier_currency'), config('settings.do_convertion'))->format()]);
